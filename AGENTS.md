...
- **Run affected tests locally before asking for review or committing a slice.** Select the changed behavior and its callers with `--only`; report the selection and result. PR CI runs the complete retained suite. Run the full suite locally when a change spans auth, server isolation, persistence, or stream lifecycle; changes shared test infrastructure, the Xcode project, or `Config/`; or has an impact you cannot bound to specific tests. A change limited to `scripts/`, docs, or `.github/` runs its own checks instead: `python3 -m unittest discover -s scripts/tests` for `scripts/test-sim`, and a real run of any other script you changed. A failing build or test becomes the current task; fix it before writing more code on top.
